  ...
  function initExtensionsWithoutApplying() {
    ...
    function resolveTypeUnchecked(artWithType, artifact) {
      ...
      if (artWithType.type.scope !== 'typeOf') {
        ...
        message( 'ref-undefined-typeof', artWithType.type.location, artifact, {}, 'Error',
    ...
    if (!kindProperties[art.kind].artifacts) {
      ...
      for (const ext of extensionsDict[name]) {
        if ([ 'context', 'service' ].includes( ext.expectedKind )) {
          ...
          message( 'extend-with-artifacts', loc, ext, { name, '#': ext.expectedKind }, 'Error', {
    ...
    if (art instanceof Array) {
      ...
      art.forEach((a) => {
        if (a.name && a.name.id && a.name.id.includes('.')) {
          message( 'syntax-invalid-ident', a.name.location, a, {}, 'Error',
    ...
    for (const langKey of Object.keys( src.i18n )) {
      ...
      for (const textKey of Object.keys( src.i18n[langKey] )) {
        ...
        if (!modelVal) {
          ...
          const message = getMessageFunction( model );
          message('i18n-different-value', sourceVal.location, null,
                  { prop: textKey, otherprop: langKey }, 'Warning');
        }
      }
    }
  }
