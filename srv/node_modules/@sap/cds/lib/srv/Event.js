...
class Event extends EventContext {
  ...
  set query(q) { if (q) super.query = q }
  get query(){
    return super.query = this._.query
  }
  ...
  get path() { // NOSONAR
    const {_} = this, p = _.path
    if (p) {
      return super.path = _fqn ([p],0,this)
    }
    const q = this.query
    if (q) {
    ...
    return super.path = _.entity && (_.entity.name || _fqn (_,'entity',this))
  }
...
const _fqn = (x,p,req) => {
  ...
  const srv = req._tx; if (srv) {
    if (!srv.model && !n.startsWith(srv.name)) return x[p] = `${srv.name}.${n}`
