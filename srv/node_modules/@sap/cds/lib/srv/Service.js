...
class Service extends EventHandlers {
  ...
  async emit (event, data) {
    let req = (
      event instanceof cds.Event ? event :
      new cds.Request (typeof event === 'object' ? event : {event,data})
    )
    let nested, tx = req._tx = (
      (nested = this._is_tx) ? this :             // already in tx
      (nested = cds.context) ? this.tx(nested) :  // join this one
      /* else */ this.tx (req, '_as_root')        // start a new one
    )
    const p = tx.dispatch (req)
    // return nested ? p : p.then (tx.commit, tx.rollback)
    if (nested) return p; else try {
      const r = await p
      return tx.commit(r)
    } catch(e) {
      await tx.rollback(e)
  ...
  insert (...args) { return this.query(INSERT(...args)) }
  create (...args) { return this.query(INSERT.into(...args)) }
  read   (...args) { return this.query(SELECT.from(...args)) }
  update (...args) { return this.query(UPDATE.entity(...args)) }
  delete (...args) { return is_rest(args[0]) ? this.send('DELETE',...args) : this.query(DELETE.from(...args)) }
...
const _reflect = (srv,filter) => !srv.model ? [] : srv.model.childrenOf (
  srv.definition && srv.definition.name || srv.model.namespace || srv.name, filter
)
