...
class ProtocolAdapter {
  ...
  */
  static serve (srv, protocol = _protocol4(srv)) {
    // REVISIT: monkey-patching srv -> move that into base implementations
    const monkey = {__proto__:srv, _emit:srv.emit, emit(eve,data) {
      if (eve._ && eve._.req) eve._.req.emit ('dispatch',eve) //> emit dispatch event for logging
      return this._emit(eve,data)
    }}
    const cached = (srv._adapters || (srv._adapters={})) [protocol]; if (cached) return cached
    const adapter = Object.defineProperties (this.at (protocol) (monkey), _prototype)
