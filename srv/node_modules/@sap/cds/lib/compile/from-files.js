...
async function cds_load (files, options) {
  const all = cds.resolve(files) || cds.error (
...
async function cds_get (files, options, _inferred) { // NOSONAR
  ...
  if (o.files || o.flavor === 'files') return cds.resolve(files)
  if (o.sources || o.flavor === 'sources') return _sources4 (cds.resolve(files))
