...
module.exports = function () {
  this.on(['CREATE', 'READ', 'UPDATE', 'DELETE'], '*', async req => {
    ...
    // no changes, no op (otherwise, @cds.on.update gets new values)
    // once we remove the existence check in the protocol adapter,
    ...
    if (req.event === 'UPDATE' && result === 0) {
      const testRead = await cds.tx(req).run(_getSelectCQN(req.query, [1]))
