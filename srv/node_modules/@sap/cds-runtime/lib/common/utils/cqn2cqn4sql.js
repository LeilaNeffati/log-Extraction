...
const _structElement = (name, entity) => {
  if (!entity) return

  if (entity.elements) {
    return entity.elements[name]
  }
}
...
const convertPathExpressionToWhere = (fromClause, model) => {
  ...
  for (let i = 0; i < fromClause.ref.length; i++) {
    ...
    const element = _structElement(fromClause.ref[i], entity)
...
const _convertInsert = (cqn, model) => {
  ...
  return resolveView(insert, model)
}
...
const _convertDelete = (cqn, model) => {
  ...
  const newDelete = resolveView(cqn, model)
