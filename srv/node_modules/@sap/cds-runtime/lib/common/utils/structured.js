...
const _flattenStructuredInExpand = (column, expandedEntity) => {
  if (column.ref.length > 1) {
    throw getError(501, '$expand on navigations in structured elements is not supported')
  }
  ...
  for (const expandElement of column.expand) {
    ...
    if (expandElement.expand) {
      _flattenStructuredInExpand(expandElement, expandedEntity.elements[propertyName]._target)
      continue
    }
...
const _flattenColumns = (SELECT, flattenedElements, toBeDeleted, csnEntity) => {
  for (const column of SELECT.columns) {
    ...
    if (element && column.expand) {
      _flattenStructuredInExpand(column, element._target)
      continue
    }
