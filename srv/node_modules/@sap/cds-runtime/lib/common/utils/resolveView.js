...
const _getTransitionData = (target, columns, force, skipForbiddenViewCheck) => {
  // REVISIT: Find less param polluting way to skip ForbiddenViewCheck for reads
...
const _checkForForbiddenViews = queryTarget => {
  ...
  if (select) {
    if (!select.from || select.from.join || select.from.length > 1 || select.where) {
...
const resolveView = (query, model, force) => {
