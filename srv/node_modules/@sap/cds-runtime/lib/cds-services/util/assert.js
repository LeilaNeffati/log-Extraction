...
const _assertError = (code, element, value, key) => {
...
const _checkStaticElementByKey = (entity, key, value, result, ignoreNonModelledData) => {
  ...
  if (!element) {
    if (!checkComplexType([key, value], entity.elements, ignoreNonModelledData)) {
      result.push(_assertError(ASSERT_VALID_ELEMENT, { name: key }))
    }
  ...
  if (check && !check(value, element)) {
    ...
    result.push(_assertError(ASSERT_DATA_TYPE, element, value, key))
  }
...
const checkInputConstraints = (element, value, result, key) => {
  ...
  if (enumElements && !_enumValues(enumElements).includes(value)) {
    result.push(_assertError(ASSERT_ENUM, element, value, key))
  }
...
const _checkAssociations = (entity, element, row, context, result) => {
  if ((context.event === 'CREATE' || context.event === 'UPDATE') && row[element.name] !== undefined) {
    if (element.is2many && !element['@odata.contained']) {
      result.push(_assertError(ASSERT_DEEP_TO_MANY_ASSOCIATION, element))
      return result
    }

    if (element.is2one && !element['@odata.contained']) {
      result.push(_assertError(ASSERT_DEEP_TO_ONE_ASSOCIATION, element))
      return result
    }
  }
}
  ...
  return data.reduce((result, row) => {
    ...
    checks.push(
      _checkExists(element._target, foreignKey, context, run).then(exists => {
        if (!exists) {
          result.push(_assertError(ASSERT_REFERENCE_INTEGRITY, element, foreignKey))
        }
      })
    )
      ...
      checks.push(
        _checkExistsWhere(source, [where], run).then(exists => {
          if (exists.includes(true)) {
            result.push(_assertError(ASSERT_REFERENCE_INTEGRITY, elements[assoc], context.data))
          }
        })
      )
  ...
  return data.reduce((result, row) => {
    for (const key of entityKeys) {
      if (entityKeys.some(key => row[key] === undefined)) {
        result.push(_assertError(ASSERT_NOT_NULL, entity.elements[key]))
      }
    }
    return result
  }, [])
